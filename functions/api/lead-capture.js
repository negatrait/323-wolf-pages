export async function onRequestPost(context) {
  ...
  try {
    ...
    const { email, url, source = 'landing-page' } = body;
    ...
    // 2. Generate audit score (if URL provided)
    let audit_score = 0;
    ...
    // 3. Generate lead_id (UUID v4 format)
    ...
    try {
      const webhookResponse = await fetch(`${env.LEAD_WEBHOOK_URL || 'https://n8n.sivussa.com'}/api/lead-webhook`, {
        method: 'POST',
    ...
    return new Response(
      JSON.stringify({
        ...
        business_type,
        priority,
        message: 'Thanks! Your audit is being prepared.'
      }),
      {
        status: 200,
